...
class UserProfile(db.Model):
    ...
    # search_count = db.Column(db.Integer, unique=False, nullable=False)
    ...
    def search(search_obj):
        ...
        results = UserProfile.query.filter(final_query).all()

        return results

    # @staticmethod
    # def query_top_searched(limit=100):
    #
    #     results = UserProfile.query.order_by(UserProfile.search_count.desc()).limit(limit).all()
    #     return results